 ...
 */
  ...
  */
 export function setMode(keywordList = [], specialRule) {
   CodeMirror.defineMode("javascript", function (config, parserConfig) {
     ...
     // { type: type, style: "keyword" }
 
     const keywords = keywordList.reduce((pre, curr) => {
       if (typeof curr === "string") {
         pre = { ...pre, [curr]: { type: `keyword ${curr}`, style: "keyword" } };
         return pre;
       }
       return { ...pre, [curr.text]: curr };
     }, {});
     ...
     function tokenBase(stream, state) {
       ...
       // 用户自定义规则(匹配成功则返回样式)
       if (typeof specialRule === "function") {
         const styleStr = specialRule(ch, stream);
         if (styleStr) {
           return styleStr;
         }
       }
       ...
       if (ch == '"' || ch == "'") {
         ...
         // 普通字符串视为错误
         return ret("variable", "error", word);
       }
     }
     ...
     // 原始函数
     function originToken(stream, state) {
       // sol 仅当流在行的开头时才返回true。
       // if (stream.sol()) {
     ...
     return {
       ...
       token: function (stream, state) {
         // stream.next()
         return originToken(stream, state);
       },
